...
// Ensure an product ID is provided
if (!isset($_GET['id'])) {
...
try {
    ...
    // Log the error and redirect or display a user-friendly message
    error_log("Database error fetching product details: " . $e->getMessage());
    echo "<p>An error occurred while fetching product details. Please try again later.</p>";
    exit();
}
    ...
    if (empty($errors)) {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if ($uploadOk == 0) {
                ...
                // Errors already added to $errors array
            } else {
                if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
                    // Delete old image if it exists and is different from the new one
                    if ($image_url && file_exists($image_url) && $image_url != $target_file) {
                        unlink($image_url);
                    }
        ...
        if (empty($errors)) {
            try {
                $stmt = $pdo->prepare("UPDATE products SET name = ?, description = ?, price = ?, stock = ?, 
                                    category = ?, brand = ?, image_url = ?, updated_at = CURRENT_TIMESTAMP 
                                    WHERE id = ?");
                $stmt->execute([$name, $description, $price, $stock, $category, $brand, $image_url, $product_id]);
...
<head>
    ...
    <title>Edit Product - Shopee Theme</title>
...
<body class="flex min-h-screen">
    ...
    <main class="flex-grow p-6">
        <header class="bg-white rounded-lg shadow-md p-4 mb-6 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-800">Edit Product: <?php echo htmlspecialchars($product['name']); ?></h1>
            <a href="admin_products.php" class="text-sm shopee-text-orange hover:underline">Back to Products</a>
        </header>
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($product['name']); ?>" required
                           class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-shopee-orange focus:border-shopee-orange sm:text-sm">
                </div>
                
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description:</label>
                    <textarea id="description" name="description" rows="4" required
                              class="mt-1 block w-full p-3 border border-gray-300 rounded-md shadow-sm focus:ring-shopee-orange focus:border-shopee-orange sm:text-sm"><?php echo htmlspecialchars($product['description']); ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <?php if ($product['image_url']): ?>
                        <img src="<?php echo htmlspecialchars($product['image_url']); ?>" alt="Current Product Image" class="w-32 h-32 object-cover rounded-md border border-gray-200 shadow-sm">
                    <?php else: ?>
                        <p class="text-gray-500 text-sm">No image uploaded for this product.</p>
                    <?php endif; ?>
                </div>
                
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-1">New Product Image (optional):</label>
                    <input type="file" id="image" name="image" accept="image/*"
                           class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-shopee-orange file:text-white hover:file:bg-orange-600">
                    <p class="text-xs text-gray-500 mt-1">Upload a new image to replace the current one. Max size: 2MB.</p>
                </div>
                
                <button type="submit" class="w-full shopee-gradient text-white py-3 px-6 rounded-md shadow-lg hover:opacity-90 transition duration-300 ease-in-out font-semibold text-lg">
                    Update Product
                </button>
            </form>
        </div>
    ...
    <script>
        $(document).ready(function() {
            // Apply active link styling for "Manage Products"
            $('.sidebar-item[href="admin_products.php"]').addClass('bg-gray-100 text-shopee-orange font-semibold');
            $('.sidebar-item[href="admin_products.php"]').find('svg').addClass('shopee-text-orange');
