...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($search_input)) {
        try {
            // Attempt to find order by order_id (which is 'id' in your table) or tracking_number
            // We use 'id' as order_id here, as per your schema.
            $stmt = $pdo->prepare("
                SELECT o.id AS order_id, o.user_id, o.total_amount, o.status AS order_status, 
                       o.shipping_address, o.payment_method, o.created_at AS order_date, o.tracking_number,
                       GROUP_CONCAT(CONCAT(p.name, ' (Qty: ', oi.quantity, ' @ Php', FORMAT(oi.price, 2), ')') SEPARATOR '<br>') AS items_list
                FROM orders o
                JOIN order_items oi ON o.id = oi.order_id
                JOIN products p ON oi.product_id = p.id
                WHERE o.id = :search_input OR o.tracking_number = :search_input
                GROUP BY o.id
                LIMIT 1
            ");
            ...
            if (!$order_details) {
                $error_message = "Order ID or tracking number not found. Please double-check your input.";
            }
        } catch (PDOException $e) {
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Status-specific colors based on your ENUM values */
        .status-ordered {
        ...
        /* Add more statuses if you expand your ENUM (e.g., cancelled, returned) */
        .status-cancelled {
            ...
            <section class="track-order-section">
                ...
                <form class="track-order-form" method="POST" action="track_orders.php">
                    ...
                    <input type="text" id="search_input" name="search_input" 
                           placeholder="e.g., <?php echo htmlspecialchars($order_details['order_id'] ?? '6'); ?> or TRK-XXXXXX" 
                           required value="<?php echo htmlspecialchars($_POST['search_input'] ?? ''); ?>">
